...
async function runTests() {
  ...
  console.log('✅ Express test server running on port 5000');
  ...
  }, { username: 'admin1_p3', password: 'admin123', pin: '784920', role: 'admin1' });

  const admin1Token = admin1Auth.data.token;
  console.log('✅ Admin1 login token acquired:', !!admin1Token);
  ...
  const admin2C1Auth = await makeRequest({
    ...
    method: 'POST',
    headers: { 'Content-Type': 'application/json' }
  }, { username: 'admin2_c1_p3', password: 'admin123', pin: '784920', role: 'admin2' });

  const admin2C1Token = admin2C1Auth.data.token;
  console.log('✅ Admin2 (Erragattugutta C1) login token acquired:', !!admin2C1Token);
  ...
  const admin2B1Auth = await makeRequest({
    ...
    method: 'POST',
    headers: { 'Content-Type': 'application/json' }
  }, { username: 'admin2_b1_p3', password: 'admin123', pin: '784920', role: 'admin2' });

  const admin2B1Token = admin2B1Auth.data.token;
  console.log('✅ Admin2 (Beemaram C1) login token acquired:', !!admin2B1Token, '\n');
